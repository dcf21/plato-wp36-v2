...
from eas_qats_wrapper import qats
...
def task_handler(execution_attempt: task_database.TaskExecutionAttempt):
    ...
    logging.info("Running <{directory}/{filename}> through QATS with duration {lc_days:.1f}.".format(
    ...
    x = qats.process_lightcurve(lc=lc_in, lc_duration=lc_duration, search_settings=search_settings)
    ...
    for item in ['integrated_transit_power', 'pixels_in_transit', 'pixels_in_transit', 'mes']:
        qats_output[item] = lc_in.metadata.get(item, None)
    ...
    with task_database.TaskDatabaseConnection() as task_db:
        ...
        task_db.execution_attempt_update(attempt_id=execution_attempt.attempt_id,
                                         metadata={**qats_output, **qc_metadata})
